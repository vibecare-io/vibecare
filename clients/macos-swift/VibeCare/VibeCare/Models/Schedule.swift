...
// MARK: - Sample Data
extension Schedule {
    static func samples(for routines: [Routine]) -> [Schedule] {
        guard !routines.isEmpty else { return [] }

        let eyeCareRoutine = routines.first { $0.name.contains("20-20-20") }
        let postureRoutine = routines.first { $0.name.contains("Posture") }
        let hydrationRoutine = routines.first { $0.name.contains("Hydration") }

        var samples: [Schedule] = []

        // Every 20 minutes during work hours (9 AM - 6 PM)
        if let routine = eyeCareRoutine {
            let rrule = RRule(
                freq: .minutely,
                interval: 20,
                byhour: Array(9...18)
            ...
            samples.append(Schedule(
                ...
                recurrenceJSON: (try? rrule.toJSON()) ?? "",
                dtstart: Calendar.current.date(bySettingHour: 9, minute: 0, second: 0, of: Date()),
                notes: "20-20-20 rule during work hours"
            ))
        }

        // Every 2 hours during the day
        if let routine = postureRoutine {
            let rrule = RRule(
                freq: .hourly,
                interval: 2,
                byhour: Array(8...20)
            ...
            samples.append(Schedule(
                ...
                name: "Every 2 hours",
                recurrenceJSON: (try? rrule.toJSON()) ?? "",
                dtstart: Calendar.current.date(bySettingHour: 8, minute: 0, second: 0, of: Date()),
                notes: "Posture check reminder"
            ))
        }

        // Every hour during work days
        if let routine = hydrationRoutine {
            let rrule = RRule(
                freq: .hourly,
                interval: 1,
                byhour: Array(9...17),
            ...
            samples.append(Schedule(
                ...
                recurrenceJSON: (try? rrule.toJSON()) ?? "",
                dtstart: Calendar.current.date(bySettingHour: 9, minute: 0, second: 0, of: Date()),
                notes: "Stay hydrated during work"
            ))
        }
